...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .container { background: #fff; border-radius: 20px; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); padding: 40px; width: 100%; max-width: 500px; }
        .header { text-align: center; margin-bottom: 30px; }
        .header h1 { color: #333; font-size: 24px; margin-bottom: 8px; }
        .header p { color: #666; font-size: 14px; }
        .form-group { margin-bottom: 20px; }
        label { display: block; margin-bottom: 8px; color: #333; font-weight: 600; font-size: 14px; }
        input, select { width: 100%; padding: 12px 15px; border: 2px solid #e1e1e1; border-radius: 8px; font-size: 14px; transition: all 0.3s ease; background: #fff; }
        input:focus, select:focus { outline: none; border-color: #667eea; box-shadow: 0 0 0 3px rgba(102, 126, 234, 0.1); }
        .logo-upload { text-align: center; margin-bottom: 25px; }
        .logo-preview { width: 100px; height: 100px; border-radius: 50%; background: #f1f1f1; margin: 0 auto 15px; display: flex; align-items: center; justify-content: center; border: 2px dashed #ccc; overflow: hidden; }
        .logo-preview img { width: 100%; height: 100%; object-fit: cover; }
        .logo-preview svg { width: 40px; height: 40px; fill: #ccc; }
        .file-input { display: none; }
        .upload-label { display: inline-block; padding: 10px 20px; background: #f1f1f1; color: #666; border-radius: 8px; cursor: pointer; font-size: 14px; transition: all 0.3s ease; }
        .upload-label:hover { background: #e1e1e1; }
        ...
        .btn-batal { background: #f1f1f1; color: #666; }
        .btn-batal:hover { background: #e1e1e1; }
        .btn-hapus { background: #ffebee; color: #e74c3c; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1>Ubah Entitas</h1>
            <p>Perbarui informasi organisasi/dinas</p>
        </div>

        <form id="entitasForm">
            <input type="hidden" id="idEntitas" name="idEntitas">
            ...
            <div class="btn-group">
                <button type="button" class="btn-hapus" onclick="hapusEntitas()">Hapus</button>
    ...
    <script>
        ...
        const idEntitas = urlParams.get('id');
        ...
        async function loadEntitas() {
            ...
            try {
                const response = await fetch('api/proses_entitas.php?id=' + idEntitas);
                const result = await response.json();

                if (result.success && result.data) {
                    const entitas = result.data;
                    document.getElementById('idEntitas').value = entitas.id_entitas;
                    document.getElementById('namaentitas').value = entitas.nama_entitas;
                    document.getElementById('level').value = entitas.level;
                    document.getElementById('daerah').value = entitas.daerah || '';
                    document.getElementById('alamat').value = entitas.alamat || '';
                    document.getElementById('telepon').value = entitas.telepon || '';
                    ...
                    Swal.fire('Error', 'Entitas tidak ditemukan', 'error')
        ...
        document.getElementById('entitasForm').addEventListener('submit', async function(e) {
            ...
            formData.append('id_entitas', document.getElementById('idEntitas').value);
            ...
            try {
                ...
                if (result.success) {
                    Swal.fire({
                        title: 'Berhasil!',
                        text: 'Entitas berhasil diperbarui',
                        icon: 'success',
